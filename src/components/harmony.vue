<template>
  <div :class="{webwallet:account}">
    ...
    <template v-else>
      <side-bar :balances="balances" :account="account" :blockchain="blockchain" :shards="shardArray" @getFromShard="getFromShard"></side-bar>
      <section class="main-info">
        <div class="main-container transfer-container">
          <div class="tabs">
            <!-- 转账tab -->
            <a class="tab">{{$t("transfer")}}</a>
            <a class="tab" target="_blank" href=" https://explorer.harmony.one/">{{$t("webwallet_transactions")}}</a>
          </div>
          <!-- 转账内容 -->
          <div class="transfer">
            <form class="basic-form">
              ...
              <ul class="basic-group clearfix">
                ...
                <li class="token">
                  ...
                  <select v-model="toShard">
                    <option v-for="item in shardArray" :value="item.shardID" :key="item.shardID">{{item.shardID}}</option>
                  </select>
                </li>
              </ul>
              <ul class="basic-group clearfix">
              ...
              <div class="input-data">
                ...
                <textarea cols="50" rows="3" :placeholder="$t('transfer_input')" v-model="transfer.input"></textarea>
              </div>
